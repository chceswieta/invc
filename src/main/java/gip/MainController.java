...
public class MainController {
	...
	public void initialize() {
		try {
			...
			boolean moreThanClientAccess = grants.contains("GRANT SELECT, INSERT, UPDATE, DELETE ON `invc`.`invoice`")
					&& grants.contains("GRANT SELECT, INSERT, UPDATE, DELETE ON `invc`.`invoiceElement`")
					&& grants.contains("GRANT SELECT, LOCK TABLES, EXECUTE ON `invc`.*");
			if (moreThanClientAccess) {
				if (grants.contains("GRANT SELECT, INSERT, UPDATE, DELETE ON `invc`.`client`")
						&& grants.contains("GRANT SELECT, INSERT, UPDATE, DELETE ON `invc`.`product`"))
					grantAccess("adminAccess");
				else if (grants.contains("GRANT SELECT, UPDATE ON `invc`.`product`")
						&& grants.contains("GRANT SELECT ON `invc`.`client`"))
					grantAccess("employeeAccess");
			} else if (grants.contains("GRANT SELECT ON `invc`.`invoiceElement`")
					&& grants.contains("GRANT EXECUTE ON PROCEDURE `invc`.`gen`")
					&& grants.contains("GRANT EXECUTE ON PROCEDURE `invc`.`num`"))
				grantAccess("clientAccess");
